...
public class TJ_DNTGAction4 implements SpellAction {//御龙在天

    @Override
    public void spellAction(ManData manData, FightingSkill skill, FightingEvents events, Battlefield battlefield) {
        // TODO Auto-generated method stub
        //新增技能 大闹天宫
        List<ManData> nomyDatas = MixDeal.getdaji(!PK_MixDeal.isPK(battlefield.BattleType)?6:3, manData.getCamp(), events, battlefield);
        if (nomyDatas.size() == 0) return;
        MixDeal.BB_DNTGG(nomyDatas, battlefield, manData, null, skill);
...
//		1305	同仇敌忾								物理攻击时有25%几率召唤本方所有在场召唤兽一起作战，本次攻击获得其它召唤兽的攻击力加成(上限15万)。
    ...
    public static int GMax(ManData manData, ManData nomyData, double jc, Battlefield battlefield) {
        jc += manData.ljv;
        if (nomyData == null) {
            return 5 + manData.ljs(jc);
        }
        int maxg = 5 + manData.ljs(jc - (nomyData.getSkillType(TypeUtil.TJ_YCDY) == null ? 0 : 15));
