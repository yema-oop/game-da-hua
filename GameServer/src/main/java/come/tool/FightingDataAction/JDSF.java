 ...
 * @date 21-06-01
 */
public class JDSF implements DataAction {
    ...
    public void analysisAction(ManData manData, FightingEvents fightingEvents, String type, Battlefield battlefield) {
        ...
        FightingSkill fightingSkill = manData.getSkillType(TypeUtil.BB_JDSF);
        ...
        FightingSkill skill_240 = null, skill_250 = null, skill_260 = null, skill_270 = null, skill_2800 = null, skill_290 = null;// 箭无虚发,一帆风顺,一鼓作气,同仇敌忾,一击毙命,横扫四方,莲火流星
        for (int i = manData.getSkills().size() - 1; i >= 0; i--) {
            ...
            if (sl.getSkilltype().equals(TypeUtil.BB_E_TCDK)) {
                ...
                zap8 = (long) (zap8 / 2.8);
            } else if (sl.getSkilltype().equals(TypeUtil.BB_JDSF)) {
                skill_290 = sl;
            }
        }

        List<ManData> zzs = null;
        if (skill_240 != null && Battlefield.isV(skill_240.getSkillgain())) {
            zzs = new ArrayList<>();
        ...
        while (noman.getStates() == 0 && g < maxg) {
            ...
            ap.add(BigDecimal.valueOf(zap + sbke));
            ...
            ace.setSkillskin("1258");
            if (skill_270 != null && Battlefield.isV(skill_270.getSkillgain())) {
                ap = ap.add(BigDecimal.valueOf(zap7));
                gjz.setText("大圣附身#2");
                ace.setSkillskin(TypeUtil.BB_E_DSFS);
            } else if (skill_2800 != null && Battlefield.isV(skill_2800.getSkillgain())) {
            ...
            if (skill_290 != null) {
                ...
                if (skill_290.getSkillhurt() > 0) {
                    lastAP = lastAP.add(lastAP.multiply(BigDecimal.valueOf(100)).divide(BigDecimal.valueOf(skill_290.getSkillhurt())));

                }
                if (Battlefield.isV(skill_290.getSkillgain())) {
                    //触发莲火流星特效改变
                    gjz.setStartState("特效1");
                    ace.setSkillskin("4018");
                    gjz.setText("剑动四方#2");
                    //触发莲火流星后溅射范围2的所有敌人
            ...
            if (skill_250 != null && Battlefield.isV(skill_250.getSkillgain())) {
            ...
            // 横扫四方
            if (skill_260 != null && (isHSSF || Battlefield.isV(skill_260.getSkillgain() / 20))) {
                ...
                for (int j = datas.size() - 1; j >= 0; j--) {
                    ...
                    if (nomyData2.getStates() != 0)
                        continue;
                    ChangeFighting acec1 = new ChangeFighting();
                    ap = PhyAttack.Hurt((manData.getAp().multiply(BigDecimal.valueOf(skill_260.getSkillgain())).divide(BigDecimal.valueOf(100D))), g, manData,
