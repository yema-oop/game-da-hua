 ...
 * @date 21-06-01
 */
public class JDBH implements DataAction {
    ...
    public void analysisAction(ManData manData, FightingEvents fightingEvents, String type, Battlefield battlefield) {
        ...
        FightingSkill fightingSkill = manData.getSkillType(TypeUtil.BB_JDBH);
        ...
        for (int i = manData.getSkills().size() - 1; i >= 0; i--) {
            ...
            if (sl.getSkilltype().equals(TypeUtil.BB_E_TCDK)) {
                ...
                zap8 = (long) (zap8 / 2.8);
            } else if (sl.getSkilltype().equals(TypeUtil.BB_JDBH)) {
                skill_9 = sl;
            }
        }
        ...
        while (noman.getStates() == 0 && g < maxg) {
            ...
            if (skill_9 != null) {
                ...
                if (skill_9.getSkillhurt() > 0) {
                    lastAP = lastAP.add(lastAP.multiply(BigDecimal.valueOf(100)).divide(BigDecimal.valueOf(skill_9.getSkillhurt())));
                }
                if (Battlefield.isV(skill_9.getSkillgain())) {
                    //触发莲火流星特效改变
                    gjz.setStartState("特效1");
                    ace.setSkillskin("802");
                    gjz.setText("如来神掌#2");
                    //触发莲火流星后溅射范围2的所有敌人
            ...
            if (skill_6 != null && (isHSSF || Battlefield.isV(skill_6.getSkillgain() / 20))) {
                ...
                for (int j = datas.size() - 1; j >= 0; j--) {
                    ...
                    if (nomyData2.getStates() != 0)
                        continue;
                    ChangeFighting acec1 = new ChangeFighting();
                    ap = PhyAttack.Hurt(manData.getAp().multiply(BigDecimal.valueOf(skill_6.getSkillgain() / 100D)), g, manData,
