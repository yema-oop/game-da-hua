 ...
 * @date 21-06-01
 */
public class BB_QDWG implements DataAction {
    ...
    public void analysisAction(ManData manData, FightingEvents fightingEvents, String type, Battlefield battlefield) {
        ...
        FightingSkill fightingSkill = manData.getSkillType(TypeUtil.BB_QDWG);
        ...
        FightingSkill skill_44 = null, skill_45 = null, skill_46 = null, skill_47 = null, skill_48 = null, skill_49 = null;// 箭无虚发,一帆风顺,一鼓作气,同仇敌忾,一击毙命,横扫四方,莲火流星
        for (int i = manData.getSkills().size() - 1; i >= 0; i--) {
            ...
            if (sl.getSkilltype().equals(TypeUtil.BB_E_TCDK)) {
                ...
                zap8 = (long) (zap8 / 2.8);
            } else if (sl.getSkilltype().equals(TypeUtil.BB_QDWG)) {
                skill_49 = sl;
            }
        }

        List<ManData> zzs = null;
        if (skill_44 != null && Battlefield.isV(skill_44.getSkillgain())) {
            zzs = new ArrayList<>();
        ...
        while (noman.getStates() == 0 && g < maxg) {
            ...
            ace.setSkillskin("1244");
            if (skill_47 != null && Battlefield.isV(skill_47.getSkillgain())) {
                ap = ap.add(BigDecimal.valueOf(zap7));
                gjz.setText("大圣附身#2");
                ace.setSkillskin(TypeUtil.BB_E_DSFS);
            } else if (skill_48 != null && Battlefield.isV(skill_48.getSkillgain())) {
            ...
            if (skill_49 != null) {
                ...
                if (skill_49.getSkillhurt() > 0) {
                    lastAP = lastAP.add(lastAP.multiply(BigDecimal.valueOf(100)).divide(BigDecimal.valueOf(skill_49.getSkillhurt())));
                }
                if (Battlefield.isV(skill_49.getSkillgain())) {
                    //触发莲火流星特效改变
                    gjz.setStartState("特效1");
                    ace.setSkillskin("4007");
                    gjz.setText("千刀万剐#2");
                    //触发莲火流星后溅射范围2的所有敌人
            ...
            if (skill_45 != null && Battlefield.isV(skill_45.getSkillgain())) {
            ...
            // 横扫四方
            if (skill_46 != null && (isHSSF || Battlefield.isV(skill_46.getSkillgain() / 20))) {
                ...
                for (int j = datas.size() - 1; j >= 0; j--) {
                    ...
                    if (nomyData2.getStates() != 0)
                        continue;
                    ChangeFighting acec1 = new ChangeFighting();
                    ap = PhyAttack.Hurt((manData.getAp().multiply(BigDecimal.valueOf(skill_46.getSkillgain())).divide(BigDecimal.valueOf(100D))), g, manData,
