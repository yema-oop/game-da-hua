 ...
 * @author BigGreen
 */
public class ZhizunShengpan implements DataAction {
    @Override
    public void analysisAction(ManData manData, FightingEvents fightingEvents, String type, Battlefield battlefield) {
        ...
        FightingSkill zzsp = manData.getSkillType(TypeUtil.BB_ZZSP);
        // 根据亲密计算是否触发
        if (Battlefield.random.nextInt(100) > zzsp.getSkillgain()) {
            return;
        }
        ...
        for (int i = 0; i < battlefield.fightingdata.size(); i++) {
            ...
            if (manData.getCamp() != battlefield.fightingdata.get(i).getCamp()) {
                AddState addState = battlefield.fightingdata.get(i).xzstate("2012");
                if (addState != null) {
        ...
        for (int j = datas.size() - 1; j >= 0; j--) {
            ...
            if (Accepter == null) {
                ...
                BigDecimal jdsh = BigDecimal.valueOf(zzsp.getSkillhurt()).multiply(BigDecimal.valueOf(manData.getLvl()).divide(BigDecimal.valueOf(15000), 0, BigDecimal.ROUND_HALF_UP).add(manData.getMp_z().multiply(BigDecimal.valueOf(manData.getLvl()).divide(BigDecimal.valueOf(1000)))));

                HurtAction.hurt(BigDecimal.valueOf( skill.getSkillhurt()), jc, jdsh.doubleValue(), kbl, skills, Accepterlist, Accepter, manData, data, skill, battlefield);
            } else {
