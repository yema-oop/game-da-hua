...
public class SplitSoul implements DataAction {
    ...
    public void analysisAction(ManData manData, FightingEvents fightingEvents,
        ...
        long sum = fightingSkill.getSkillsum() + (Battlefield.random.nextInt(100) < fightingSkill.getSkillgain() ? 1 : 0);
            ...
            while (data.getStates() == 0 && g < maxg) {
                ...
                if (i == 0 && skill_6 != null && (isHSSF || Battlefield.isV(skill_6.getSkillgain() / 20))) {
                    ...
                    for (int j = datas.size() - 1; j >= 0; j--) {
                        ...
                        ap = PhyAttack.Hurt((manData.getAp().multiply(BigDecimal.valueOf(skill_6.getSkillgain()).divide(BigDecimal.valueOf(100D)).add(BigDecimal.valueOf(sbke)))), g, manData, nomyData2, type, ace1, zls, battlefield, 0, 0);
