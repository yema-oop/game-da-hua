 ...
 * @date 21-06-01
 */
public class WJGZ implements DataAction {
    ...
    public void analysisAction(ManData manData, FightingEvents fightingEvents, String type, Battlefield battlefield) {
        ...
        FightingSkill fightingSkill = manData.getSkillType(TypeUtil.BB_WJGZ);
        ...
        FightingSkill skill_14 = null, skill_15 = null, skill_16 = null, skill_17 = null, skill_18 = null, skill_19 = null;// 箭无虚发,一帆风顺,一鼓作气,同仇敌忾,一击毙命,横扫四方,莲火流星
        for (int i = manData.getSkills().size() - 1; i >= 0; i--) {
            ...
            if (sl.getSkilltype().equals(TypeUtil.BB_E_TCDK)) {
                ...
                zap8 = (long) (zap8 / 2.8);
            } else if (sl.getSkilltype().equals(TypeUtil.BB_WJGZ)) {
                skill_19 = sl;
            }
        }

        List<ManData> zzs = null;
        if (skill_14 != null && Battlefield.isV(skill_14.getSkillgain())) {
            zzs = new ArrayList<>();
            ...
            if (zap.longValue() >= 150000) {
                zap = BigDecimal.valueOf(150000);
            }
        }
        ...
        // 记录最后一次伤害的数值
        BigDecimal lastAP = manData.getAp();
        // 是否追击
        boolean zhui = false;
        // 是否分裂
        boolean fen = false;
        ...
        while (noman.getStates() == 0 && g < maxg) {
            ...
            // 莲生原地射箭
            /**
             FightingState gjz12 = new FightingState();
             gjz12.setCamp(manData.getCamp());
             gjz12.setSkillsy(fightingSkill.getSkillname());
             gjz12.setMan(manData.getMan());
             gjz12.setText(fightingSkill.getSkillname());
             gjz12.setStartState("特效2"); // 特效2时间稍短 特效2<法术攻击<特效1
             fightingEvents.setAccepterlist(list);
             battlefield.NewEvents.add(fightingEvents);
             */
            //着一端射箭速度贼快
            FightingState gjz12 = new FightingState();
            gjz12.setStartState("特技2");
            gjz12.setSkillsy(fightingSkill.getSkillname());
            gjz12.setCamp(manData.getCamp());
            gjz12.setMan(manData.getMan());
            gjz12.setText(fightingSkill.getSkillname());
            ...
            if (zhui) {
                ...
                if (manData.executeNbkj(nbkj++)) {
                    FightingState gjz122 = new FightingState();
                    gjz122.setCamp(manData.getCamp());
                    gjz122.setMan(manData.getMan());;
                    gjz122.setText("看我的#G怒不可揭");
                    gjz122.setStartState("代价");
                    list.add(0,gjz122);
                }
                gjz12.setSkillskin("1831");
            } else if (fen) {
                // 怒不可揭判定
                if (manData.executeNbkj(nbkj++)) {
                    FightingState gjz122 = new FightingState();
                ...
                gjz12.setSkillskin("1832");
            } else if (zy) {
                // 怒不可揭判定
                if (manData.executeNbkj(nbkj++)) {
                    FightingState gjz123 = new FightingState();
                    gjz123.setCamp(manData.getCamp());
                    gjz123.setMan(manData.getMan());;
                    gjz123.setText("看我的#G怒不可揭");
                    gjz123.setStartState("代价");
                    list.add(0,gjz123);
                }
                FightingState gjz122 = new FightingState();
                gjz122.setCamp(manData.getCamp());
                gjz122.setMan(manData.getMan());;
                gjz122.setText("看我的#G冲冠一怒");
                gjz122.setStartState("代价");
                list.add(0,gjz122);
                gjz12.setSkillskin("cgyn");
            }
            ...
            BigDecimal multiply = manData.getAp().multiply(BigDecimal.valueOf(1.2));
            ap = BigDecimal.valueOf((double) Math.ceil(multiply.doubleValue()));
            ap.add(BigDecimal.valueOf( sbke)).add(zap);
            ...
            ace.setSkillskin("1243");
            if (skill_17 != null && Battlefield.isV(skill_17.getSkillgain())) {
                ap = ap.add(BigDecimal.valueOf(zap7));
                gjz12.setText("大圣附身#2");
                ace.setSkillskin(TypeUtil.BB_E_DSFS);
            } else if (skill_18 != null && Battlefield.isV(skill_18.getSkillgain())) {
                ap = ap.add(BigDecimal.valueOf(zap8));
                gjz12.setText("乾坤一掷#2");
                ace.setSkillskin(TypeUtil.BB_E_QKYZ);
            }
            ap = PhyAttack.Hurt(ap, g, manData, noman, type, ace,zls, battlefield,10,10);
            lastAP = ap.divide(BigDecimal.valueOf(4));

            // 灵犀-化险为夷
            manData.addDun(ap.longValue(), gjz12);

            if (skill_19 != null) {
                // 携带莲火流星觉醒技溅射伤害增加25%，这里用乘法计算，大概为原始伤害的56%（此溅射伤害不计算物理抗性）这个不是死亡时的溅射，是兽装溅射
                if (skill_19.getSkillhurt() > 0) {
                    lastAP = lastAP.add(lastAP.multiply(BigDecimal.valueOf(100)).divide(BigDecimal.valueOf(skill_19.getSkillhurt())));
                }
                if (Battlefield.isV(skill_19.getSkillgain())) {
                    //触发莲火流星特效改变
                    gjz12.setStartState("特效1");
                    ace.setSkillskin("4006");
                    gjz12.setText("万剑归宗#2");
                    //触发莲火流星后溅射范围2的所有敌人
                    ...
                    for (int i = datas.size() - 1; i >= 0; i--) {
                        ...
                        BigDecimal currAp = PhyAttack.Hurt(lastAP, g, manData, nomyData2, type, ace1, zls, battlefield, 10, 10);
                        acec1.setChangehp(currAp.negate());
            ...
            if (skill_15 != null && Battlefield.isV(skill_15.getSkillgain())) {
            ...
            // 横扫四方
            if (skill_16 != null && (isHSSF || Battlefield.isV(skill_16.getSkillgain() / 20))) {
                ...
                for (int j = datas.size() - 1; j >= 0; j--) {
                    ...
                    if (nomyData2.getStates() != 0)
                        continue;
                    ChangeFighting acec1 = new ChangeFighting();
                    ap = PhyAttack.Hurt((manData.getAp().multiply(BigDecimal.valueOf(skill_16.getSkillgain())).divide(BigDecimal.valueOf(100D))), g, manData,
            ...
            if (noman.getStates() != 0) {  // 0
                ...
                if ((nocamp == manData.getCamp() || nocamp == -1)) {
                    ...
                    for (int i = datas.size() - 1; i >= 0; i--) {
                        ...
                        acec1.setChangehp(lastAP.negate());
