 ...
 * @date 21-06-01
 */
public class TSJL implements DataAction {
    ...
    public void analysisAction(ManData manData, FightingEvents fightingEvents, String type, Battlefield battlefield) {
        ...
        FightingSkill fightingSkill = manData.getSkillType(TypeUtil.BB_TSJL);
        ...
        FightingSkill skill_214 = null, skill_215 = null, skill_216 = null, skill_217 = null, skill_218 = null, skill_219 = null;// 箭无虚发,一帆风顺,一鼓作气,同仇敌忾,一击毙命,横扫四方,莲火流星
        for (int i = manData.getSkills().size() - 1; i >= 0; i--) {
            ...
            if (sl.getSkilltype().equals(TypeUtil.BB_E_TCDK)) {
                ...
                zap8 = (long) (zap8 / 2.8);
            } else if (sl.getSkilltype().equals(TypeUtil.BB_TSJL)) {
                skill_219 = sl;
            }
        }

        List<ManData> zzs = null;
        if (skill_214 != null && Battlefield.isV(skill_214.getSkillgain())) {
            zzs = new ArrayList<>();
        ...
        while (noman.getStates() == 0 && g < maxg) {
            ...
            ap = BigDecimal.valueOf(Math.ceil(manData.getAp().multiply(BigDecimal.valueOf(1.2)).doubleValue()));
            ap = ap.add(zap).add(BigDecimal.valueOf(sbke));
            ...
            ace.setSkillskin("1257");
            if (skill_217 != null && Battlefield.isV(skill_217.getSkillgain())) {
                ap = ap.add(BigDecimal.valueOf(zap7));
                gjz.setText("大圣附身#2");
                ace.setSkillskin(TypeUtil.BB_E_DSFS);
            } else if (skill_218 != null && Battlefield.isV(skill_218.getSkillgain())) {
            ...
            if (skill_219 != null) {
                ...
                if (skill_219.getSkillhurt() > 0) {
                    lastAP = lastAP.add(lastAP.multiply(BigDecimal.valueOf(100)).divide(BigDecimal.valueOf(skill_219.getSkillhurt())));
                }
                if (Battlefield.isV(skill_219.getSkillgain())) {
                    //触发莲火流星特效改变
                    gjz.setStartState("特效1");
                    ace.setSkillskin("4017");
                    gjz.setText("天神降临#2");
                    //触发莲火流星后溅射范围2的所有敌人
            ...
            if (skill_215 != null && Battlefield.isV(skill_215.getSkillgain())) {
            ...
            // 横扫四方
            if (skill_216 != null && (isHSSF || Battlefield.isV(skill_216.getSkillgain() / 20))) {
                ...
                for (int j = datas.size() - 1; j >= 0; j--) {
                    ...
                    if (nomyData2.getStates() != 0)
                        continue;
                    ChangeFighting acec1 = new ChangeFighting();
                    ap = PhyAttack.Hurt(manData.getAp().multiply(BigDecimal.valueOf(skill_216.getSkillgain() / 100D)), g, manData,
